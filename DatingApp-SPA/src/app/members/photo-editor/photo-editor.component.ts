...
deletePhoto(id: number) {
  this.alertify.confirm('êtes vous sûrs de vouloir supprimer la photo?' , () => {
    this.userService.deletePhoto(this.authService.decodeToken.nameid, id).subscribe(() => {
this.photos.splice(this.photos.findIndex(p => p.id === id), 1);
this.alertify.success('this photo are delete with success');
    }, error => {
      this.alertify.error('Erreur lors de la supression');
    });
  });
}
